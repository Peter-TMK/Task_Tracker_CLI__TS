...
switch (command) {
  ...
  case "list":
    ...
    //   break;
    // }
    const listTask = listTasks(status as "todo" | "in-progress" | "done");
    ...
    if (listTasks.length === 0) {
      ...
      listTask.forEach((task) => {
        console.log(
          `ID: ${task.id}, Description: ${task.description}, Status: ${task.status}, Created At: ${task.createdAt}`
        );
      });
    }
    break;
  case "update":
    ...
    updateTask(id, updateDescription);
